    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    new_cell = (i, j)
                    if not new_cell in self.safes and not new_cell in self.mines:
            ...
            for sentence2 in self.knowledge:
                if sentence1 != sentence2:
                    if sentence1.cells.issubset(sentence2.cells):
                        cells = set()
                        for check_cell in sentence2.cells:
                            if not check_cell in sentence1.cells:
                                cells.add(check_cell)
                        sentence = Sentence(cells, sentence2.count - sentence1.count)
